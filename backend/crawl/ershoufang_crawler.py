...
import requests
import datetime
from config import city_max_page, get_redis_conn, ershoufang_city_house_urls_key, logger
...
class ErShouFangCrawler(LianjiaCrawler):
    ...
    def __init__(self):
        ...
        for city, city_url in self.city_dict.items():
            ...
            house_url = ['https://m.lianjia.com/{}/ershoufang/{}.html'.format(city_url, id) for id in house_url_ids]
            self.ershoufang_house_urls[city] = house_url
        redis_connection = get_redis_conn()
        redis_connection.set(ershoufang_city_house_urls_key, json.dumps(self.ershoufang_house_urls))
        self.get_house_data()
    ...
    def get_total_page(self, city_url):
        ...
        try:
            ...
            print("total_page {}".format(page))
            if isinstance(page, int):
                return int(int(page) / 30) + 1
            return city_max_page
        except:
    ...
    def get_house_data(self):
        ...
        for city, house_urls in self.ershoufang_house_urls.items():
            ...
            statistic['total'] = len(house_urls)
            statistic['city'] = city
