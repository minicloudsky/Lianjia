...
class RegisterView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        register_user.save()
        if register_user:
            return Response(status=status.HTTP_201_CREATED, data={"code": 200, "msg": "注册成功", "data": {"user": register_user.username}})
...
class AuthenticatePasswordView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        password = data.get('password', '')
        if not password:
            return Response(status=status.HTTP_200_OK, data={"code": 200, "msg": '密码不能为空', 'data': {"is_valid_password": 0}})
        user = authenticate(request, username=username, password=password)
        if not user:
            return Response(status=status.HTTP_200_OK, data={"code": 200, "msg": '密码错误', 'data': {"is_valid_password": 0}})

        return Response(status=status.HTTP_202_ACCEPTED, data={"code": 200, "msg": "密码验证成功", "data": {"is_valid_password": 1}})
...
class ResetPasswordView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not username or not password or not new_password:
            return Response(status=status.HTTP_200_OK, data={"code": 200, "msg": '用户名或密码不能为空', 'data': {"is_valid_password": 0}})
        user = authenticate(request, username=username, password=password)
        if not user:
            return Response(status=status.HTTP_200_OK, data={"code": 200, "msg": '密码错误', 'data': {"is_valid_password": 0}})
